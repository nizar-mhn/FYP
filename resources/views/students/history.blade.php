                ...
                @if (count($printingInfoID))
                    ...
                    @foreach ($printingInfoID as $id)
                        @php
                            ...
                            $orderInfo = DB::table('order_printing_infos')
                                ->where('orderPrintingInfoID', $id->orderPrintingInfoID)
                                ->first();
                            $file = DB::table('files')
                                ->where('fileID', $orderInfo->fileID)
                                ->first();
                            $payment = Payment::where('orderID', $id->orderID)->first();
                        @endphp
                        <div class="row mt-3 align-items-center orderDetails"
                            style="background-color: #2A9D8F; border-radius:22px">
                            <div class="col-md-2">
                                <img src="data:image/png;base64,{{ $file->thumbnail }}" alt=""
                                    class="img-fluid p-1 thumbnail border border-2">
                            </div>
